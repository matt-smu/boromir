...
def setup_test_dir(*args, **kwargs):
  ...
  if pathlib.Path(INPUT_FILE).exists():
    input_p = pathlib.Path(INPUT_FILE).read_text()
  else:
...
"""
  ...
  outfile = SEP.join((WORKING_DIR, INPUT_FILE_NAME))
...
def Main():
  """Run MulVal from python.

  MulVal entry point graph_gen.sh has 2 main functional blocks. In this module
  these are:
  1.  graph_gen.runMulVal() runs the datalog model in XSB and creates a trace.P
  2.  attack_gen.render() reads teh trace.P and creates and AttackGraph.dot

  Arguments:
    Mostly the same flags as MulVal accepts. Run with --help for details.
  ...
  if mulval_args:
    mulval_args = vars(mulval_args)
    logging.debug('Parsed mulval args: %s' % mulval_args)
  if attackgraph_args:
    attackgraph_args = vars(attackgraph_args)
    logging.debug('Parsed attack graph args: %s' % attackgraph_args)
  ...
  ## graph_gen.sh
  ####
  gg = graph_gen(**mulval_args)
  gg.graph_gen()

  #####
  ## attack_graph.cpp
  ####
  ag = attack_graph(attackgraph_args)
  ...
  subprocess.call(['Rscript', DATA_DIR + '/mcsim.r', '--input=' + MATRIX_FILE,
                   '--output=' + RESULTS_DIR, '--label=' + INPUT_BASE_NAME])
