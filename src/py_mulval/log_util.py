...
class _ThreadData(threading.local):
  def __init__(self):
    self.mulpy_thread_log_context = ThreadLogContext()
...
def SetThreadLogContext(thread_log_context):
  ...
  Args:
    thread_log_context: A ThreadLogContext to be written to thread local
      storage.
  """
  thread_local.mulpy_thread_log_context = thread_log_context
...
def GetThreadLogContext():
  ...
  """
  return thread_local.mulpy_thread_log_context


class LogFilter(logging.Filter):
  """Filter that injects a thread's ThreadLogContext label into log messages.

  Sets the LogRecord's mulpy_label attribute with the ThreadLogContext label.
  """

  def filter(self, record):
    record.mulpy_label = GetThreadLogContext().label
    return True
...
def ConfigureLogging(stderr_log_level, log_path, run_uri,
  ...
  stderr_format = ('%(asctime)s {} %(threadName)s %(mulpy_label)s'
                   '%(levelname)-8s %(message)s').format(run_uri)
  stderr_color_format = ('%(log_color)s%(asctime)s {} %(threadName)s '
                         '%(mulpy_label)s%(levelname)-8s%(reset)s '
                         '%(message)s').format(run_uri)
  file_format = ('%(asctime)s {} %(threadName)s %(mulpy_label)s'
  ...
  # initialized to use the LogFilter, and it is used to derive the
